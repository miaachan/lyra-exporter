// ==UserScript==
// @name         Lyra Exporter Fetch (Local Dev)
...
// @namespace    userscript://lyra-conversation-exporter-local-dev
// @version      8.2
...
// ==/UserScript==
    ...
    (function() {
        ...
        const Config = {
            ...
            TIMING: {
                ...
                CHATGPT_AUTOSYNC_DEBOUNCE: 1600,
                CHATGPT_URL_CHECK_INTERVAL: 1200
            }
        };
        ...
        const parseJSONStorage = (key, fallback) => {
            try {
                ...
                console.warn(`[Lyra] Failed to parse storage key: ${key}`, error);
                return fallback;
            }
        };
        ...
        const State = {
            ...
            chatgptCurrentWorkspaceId: localStorage.getItem('lyraChatGPTCurrentWorkspaceId') || '',
            chatgptWorkspaceLastSeenAt: localStorage.getItem('lyraChatGPTWorkspaceLastSeenAt') || '',
            chatgptWorkspaceCandidates: parseJSONStorage('lyraChatGPTWorkspaceCandidates', []),
            chatgptLastHeaderWorkspaceId: localStorage.getItem('lyraChatGPTLastHeaderWorkspaceId') || '',
            chatgptLastSettingsWorkspaceId: localStorage.getItem('lyraChatGPTLastSettingsWorkspaceId') || '',
            chatgptAutoSyncEnabled: localStorage.getItem('lyraChatGPTAutoSyncEnabled') !== 'false',
            chatgptAutoSyncDebounceTimer: null,
            chatgptAutoSyncUrlCheckTimer: null,
            chatgptAutoSyncLastUrl: '',
            chatgptAutoSyncInFlight: false,
            chatgptAutoSyncPending: false,
            chatgptAutoSyncLastHashByConversation: {},
            chatgptAutoSyncObserver: null,
            lyraExporterWindow: null,
            panelInjected: false,
        ...
        const i18n = {
            languages: {
                zh: {
                    ...
                    workspaceAutoApplied: '已自动设置工作区ID',
                    workspaceSelectionHint: '检测到多个工作区，请输入序号选择:\n',
                    workspaceSwitchCancelled: '已取消切换工作区',
                    noWorkspaceDetected: '未检测到可用工作区，已回退手动输入',
                    personalWorkspaceHint: '个人空间无需工作区ID',
                    autoSync: '自动同步',
                    autoSyncOn: '自动同步已开启',
                    autoSyncOff: '自动同步已关闭',
                    viewOnline: '预览对话',
                ...
                en: {
                    ...
                    workspaceAutoApplied: 'Workspace auto-detected and applied',
                    workspaceSelectionHint: 'Multiple workspaces found. Enter number to select:\n',
                    workspaceSwitchCancelled: 'Workspace switch cancelled',
                    noWorkspaceDetected: 'No workspace detected, fallback to manual input',
                    personalWorkspaceHint: 'Personal mode does not require workspace ID',
                    autoSync: 'Auto Sync',
    ...
    function extractGlobalCanvasContent() {
        ...
        const LyraCommunicator = {
            getOrOpenWindow: () => {
                if (State.lyraExporterWindow && !State.lyraExporterWindow.closed) {
                    return State.lyraExporterWindow;
                }
                const newWindow = window.open(Config.EXPORTER_URL, '_blank');
                if (newWindow) {
                    State.lyraExporterWindow = newWindow;
                }
                return newWindow;
            },
            open: async (jsonData, filename, extraData = {}) => {
                try {
                    ...
                    if (isExtension) {
                        ...
                        // Userscript 模式：使用单实例 window + postMessage
                        const exporterWindow = LyraCommunicator.getOrOpenWindow();
                        if (!exporterWindow) {
    ...
    const ChatGPTHandler = {
        init: () => {
            ...
            const extractWorkspaceIdFromSettingsUrl = (url) => {
                if (typeof url !== 'string') return null;
                const match = url.match(/\/backend-api\/accounts\/([a-f0-9-]+)\/settings/i);
            ...
            window.fetch = async function(resource, options) {
                ...
                const headers = options?.headers || (resource instanceof Request ? resource.headers : null);
                const authHeader = extractHeaderValue(headers, 'Authorization');
                const accountIdHeader = extractHeaderValue(headers, 'ChatGPT-Account-Id');
                ...
                if (accountIdHeader) {
                    ChatGPTHandler.recordWorkspaceCandidate(accountIdHeader, {
                        source: 'fetch_header',
                        markAsCurrent: true
                    });
                }

                const workspaceFromUrl = extractWorkspaceIdFromSettingsUrl(requestUrl);
                if (workspaceFromUrl) {
                    ChatGPTHandler.recordWorkspaceCandidate(workspaceFromUrl, {
                        source: 'fetch_url'
                    });
                }
                ...
                if (workspaceFromUrl && response?.ok) {
                    try {
                        const json = await response.clone().json();
                        if (json?.workspace_id) {
                            ChatGPTHandler.recordWorkspaceCandidate(json.workspace_id, {
                                source: 'fetch_response',
                                markAsCurrent: true
                            });
                            localStorage.setItem('lyraChatGPTLastSettingsWorkspaceId', json.workspace_id);
                        }
                    } catch (error) {
                        // Ignore non-JSON settings responses
                    }
                }
            ...
            XMLHttpRequest.prototype.open = function(method, url) {
                this.__lyraUrl = typeof url === 'string' ? url : '';
                this.__lyraWorkspaceHeader = '';
                const workspaceId = extractWorkspaceIdFromSettingsUrl(this.__lyraUrl);
                if (workspaceId) {
                    ChatGPTHandler.recordWorkspaceCandidate(workspaceId, { source: 'xhr_url' });
                }
                return rawXHROpen.apply(this, arguments);
            };

            XMLHttpRequest.prototype.setRequestHeader = function(header, value) {
                if (typeof header === 'string' && header.toLowerCase() === 'chatgpt-account-id' && value) {
                    this.__lyraWorkspaceHeader = String(value);
                    ChatGPTHandler.recordWorkspaceCandidate(value, {
                        source: 'xhr_header',
                        markAsCurrent: true
                    });
                }
            ...
            XMLHttpRequest.prototype.send = function() {
                this.addEventListener('load', function() {
                    try {
                        const workspaceIdFromUrl = extractWorkspaceIdFromSettingsUrl(this.__lyraUrl || '');
                        if (!workspaceIdFromUrl) return;

                        ChatGPTHandler.recordWorkspaceCandidate(workspaceIdFromUrl, { source: 'xhr_url' });
            ...
            State.chatgptAutoSyncLastUrl = window.location.href;
            ChatGPTHandler.bootstrapWorkspaceAutoDetect(false);
            ChatGPTHandler.startAutoSync();
        },
        ...
        normalizeWorkspaceId: (value) => {
            ...
            return /^[a-f0-9-]{30,}$/i.test(trimmed) ? trimmed : '';
        },

        persistWorkspaceCandidates: () => {
            localStorage.setItem('lyraChatGPTWorkspaceCandidates', JSON.stringify(State.chatgptWorkspaceCandidates || []));
        },

        recordWorkspaceCandidate: (workspaceId, options = {}) => {
            const normalized = ChatGPTHandler.normalizeWorkspaceId(workspaceId);
            if (!normalized) return false;

            const now = new Date().toISOString();
            const source = options.source || 'unknown';
            const markAsCurrent = options.markAsCurrent === true;
            const name = options.name || '';

            const candidates = Array.isArray(State.chatgptWorkspaceCandidates) ? [...State.chatgptWorkspaceCandidates] : [];
            const existingIndex = candidates.findIndex(item => item.id === normalized);
            ...
            if (existingIndex >= 0) {
                candidates[existingIndex] = {
                    ...
                    id: normalized,
                    source,
                    name: name || candidates[existingIndex].name || '',
                    lastSeenAt: now
                };
            } else {
                candidates.push({
            ...
            candidates.sort((a, b) => new Date(b.lastSeenAt || 0).getTime() - new Date(a.lastSeenAt || 0).getTime());
            State.chatgptWorkspaceCandidates = candidates.slice(0, 30);
            ChatGPTHandler.persistWorkspaceCandidates();

            if (source.includes('header')) {
                State.chatgptLastHeaderWorkspaceId = normalized;
                localStorage.setItem('lyraChatGPTLastHeaderWorkspaceId', normalized);
            }
            if (source.includes('response')) {
                State.chatgptLastSettingsWorkspaceId = normalized;
                localStorage.setItem('lyraChatGPTLastSettingsWorkspaceId', normalized);
            }

            if (markAsCurrent || !State.chatgptCurrentWorkspaceId) {
                State.chatgptCurrentWorkspaceId = normalized;
                State.chatgptWorkspaceLastSeenAt = now;
                localStorage.setItem('lyraChatGPTCurrentWorkspaceId', normalized);
                localStorage.setItem('lyraChatGPTWorkspaceLastSeenAt', now);
            }

            if (markAsCurrent) {
                State.chatgptWorkspaceId = normalized;
                State.chatgptWorkspaceType = 'team';
                localStorage.setItem('lyraChatGPTWorkspaceId', normalized);
                localStorage.setItem('lyraChatGPTWorkspaceType', 'team');
            }

            return true;
        },

        applyWorkspaceSelection: (workspaceId, options = {}) => {
            const normalized = ChatGPTHandler.normalizeWorkspaceId(workspaceId);
            if (!normalized) return false;

            ChatGPTHandler.recordWorkspaceCandidate(normalized, {
                source: options.source || 'manual_apply',
                markAsCurrent: true,
                name: options.name || ''
            });
            ...
            if (options.showAlert) {
                alert(options.alertText || `${i18n.t('workspaceIdSaved')}\n${normalized}`);
            }

            return true;
        },

        bootstrapWorkspaceAutoDetect: (showAlert = false) => {
            const byHeader = ChatGPTHandler.normalizeWorkspaceId(State.chatgptLastHeaderWorkspaceId || State.chatgptCurrentWorkspaceId);
            const bySettings = ChatGPTHandler.normalizeWorkspaceId(State.chatgptLastSettingsWorkspaceId);
            const saved = ChatGPTHandler.normalizeWorkspaceId(localStorage.getItem('lyraChatGPTWorkspaceId') || '');
            const byCandidates = ChatGPTHandler.normalizeWorkspaceId(State.chatgptWorkspaceCandidates?.[0]?.id || '');

            const ordered = [];
            [byHeader, bySettings, saved, byCandidates].forEach(id => {
                if (id && !ordered.includes(id)) ordered.push(id);
            });

            if (ordered.length === 0) return false;

            const applied = ChatGPTHandler.applyWorkspaceSelection(ordered[0], {
                source: 'auto_bootstrap',
                showAlert,
                alertText: `${i18n.t('workspaceAutoApplied')}\n${ordered[0]}`
            });

            if (applied) {
                setTimeout(() => UI.recreatePanel(), 0);
            }

            return applied;
        },

        getWorkspaceSelectionPrompt: (candidates) => {
            const lines = candidates.map((item, index) => {
                const name = item.name ? ` (${item.name})` : '';
                return `${index + 1}. ${item.id}${name}`;
            });
            return `${i18n.t('workspaceSelectionHint')}\n${lines.join('\n')}`;
        },

        fetchWorkspaceCandidates: async () => {
            const token = await ChatGPTHandler.ensureAccessToken();
            const deviceId = ChatGPTHandler.getOaiDeviceId();
            if (!token || !deviceId) {
                return State.chatgptWorkspaceCandidates || [];
            }
            ...
            try {
                const response = await fetch('/backend-api/accounts', { headers });
                if (!response.ok) return State.chatgptWorkspaceCandidates || [];
                const payload = await response.json();
                const accounts = Array.isArray(payload) ? payload :
                    (Array.isArray(payload?.accounts) ? payload.accounts : []);

                accounts.forEach(account => {
                    const id = account?.workspace_id || account?.id || account?.account_id;
                    const name = account?.name || account?.workspace_name || account?.display_name || '';
                    if (id) {
                        ChatGPTHandler.recordWorkspaceCandidate(id, {
                            source: 'accounts_api',
                            name
                        });
                    }
                ...
                console.warn('[ChatGPT] Failed to fetch /backend-api/accounts', error);
            }
        ...
        switchWorkspace: async () => {
            const refreshed = await ChatGPTHandler.fetchWorkspaceCandidates();
            const candidates = Array.isArray(refreshed) ? refreshed : [];

            if (candidates.length === 0) {
                alert(i18n.t('noWorkspaceDetected'));
                const newId = prompt(i18n.t('enterWorkspaceId'), State.chatgptWorkspaceId || State.chatgptCurrentWorkspaceId || '');
                if (newId?.trim()) {
                    ChatGPTHandler.applyWorkspaceSelection(newId.trim(), {
            ...
            if (candidates.length === 1) {
                ChatGPTHandler.applyWorkspaceSelection(candidates[0].id, {
            ...
            const userInput = prompt(ChatGPTHandler.getWorkspaceSelectionPrompt(candidates), '1');
            if (userInput === null) {
                alert(i18n.t('workspaceSwitchCancelled'));
                return;
            }

            const selectedIndex = parseInt(userInput.trim(), 10);
            ...
            ChatGPTHandler.applyWorkspaceSelection(selected.id, {
        ...
        scheduleAutoSync: (reason = 'mutation', delay = Config.TIMING.CHATGPT_AUTOSYNC_DEBOUNCE) => {
            if (!State.chatgptAutoSyncEnabled) return;
            if (State.chatgptAutoSyncDebounceTimer) {
                clearTimeout(State.chatgptAutoSyncDebounceTimer);
            }
            State.chatgptAutoSyncDebounceTimer = setTimeout(() => {
                ChatGPTHandler.syncCurrentConversationToLyra(reason);
            }, delay);
        },

        syncCurrentConversationToLyra: async (reason = 'manual') => {
            if (!State.chatgptAutoSyncEnabled) return;
            const conversationId = ChatGPTHandler.getCurrentConversationId();
            if (!conversationId) return;

            if (State.chatgptAutoSyncInFlight) {
                State.chatgptAutoSyncPending = true;
                return;
            }

            State.chatgptAutoSyncInFlight = true;
            ...
            try {
                ...
                const jsonString = JSON.stringify(data, null, 2);
                const payloadHash = simpleHash(jsonString);
                const prevHash = State.chatgptAutoSyncLastHashByConversation[conversationId];

                if (prevHash === payloadHash) {
                    return;
                }

                State.chatgptAutoSyncLastHashByConversation[conversationId] = payloadHash;
                const filename = `chatgpt_live_${conversationId}.json`;

                await LyraCommunicator.open(jsonString, filename, {
                    replaceExisting: true,
                    upsertKey: `chatgpt_live_${conversationId}`,
                    syncedAt: new Date().toISOString(),
                    autoSync: true,
                    reason
                });
            } catch (error) {
                console.error('[ChatGPT] Auto sync failed:', error);
            } finally {
                State.chatgptAutoSyncInFlight = false;
                if (State.chatgptAutoSyncPending) {
                    State.chatgptAutoSyncPending = false;
                    ChatGPTHandler.scheduleAutoSync('pending', 600);
                }
            }
        },
        ...
        stopAutoSync: () => {
            if (State.chatgptAutoSyncDebounceTimer) {
                clearTimeout(State.chatgptAutoSyncDebounceTimer);
                State.chatgptAutoSyncDebounceTimer = null;
            }
            if (State.chatgptAutoSyncUrlCheckTimer) {
                clearInterval(State.chatgptAutoSyncUrlCheckTimer);
                State.chatgptAutoSyncUrlCheckTimer = null;
            }
            if (State.chatgptAutoSyncObserver) {
                State.chatgptAutoSyncObserver.disconnect();
                State.chatgptAutoSyncObserver = null;
            }
        },

        startAutoSync: () => {
            ChatGPTHandler.stopAutoSync();
            if (!State.chatgptAutoSyncEnabled) return;

            State.chatgptAutoSyncObserver = new MutationObserver(() => {
                ChatGPTHandler.scheduleAutoSync('mutation');
            });
            State.chatgptAutoSyncObserver.observe(document.body, {
                childList: true,
                subtree: true,
                characterData: true
            });
            ...
            State.chatgptAutoSyncLastUrl = window.location.href;
            State.chatgptAutoSyncUrlCheckTimer = setInterval(() => {
                const nowUrl = window.location.href;
                if (nowUrl !== State.chatgptAutoSyncLastUrl) {
                    State.chatgptAutoSyncLastUrl = nowUrl;
                    ChatGPTHandler.bootstrapWorkspaceAutoDetect(false);
                    ChatGPTHandler.scheduleAutoSync('url_change', 1000);
                }
            }, Config.TIMING.CHATGPT_URL_CHECK_INTERVAL);

            ChatGPTHandler.scheduleAutoSync('init', 1800);
        },
        ...
        addUI: (controls) => {
            ...
            const autoSyncToggle = Utils.createToggle(
                i18n.t('autoSync'),
                Config.AUTO_SYNC_ID,
                State.chatgptAutoSyncEnabled
            );

            const autoSyncInput = autoSyncToggle.querySelector('input');
            autoSyncInput.addEventListener('change', (e) => {
                State.chatgptAutoSyncEnabled = e.target.checked;
                localStorage.setItem('lyraChatGPTAutoSyncEnabled', State.chatgptAutoSyncEnabled ? 'true' : 'false');
                if (State.chatgptAutoSyncEnabled) {
                    ChatGPTHandler.startAutoSync();
                    console.log('[ChatGPT] Auto sync enabled');
                } else {
                    ChatGPTHandler.stopAutoSync();
                    console.log('[ChatGPT] Auto sync disabled');
                }
            });
            controls.appendChild(autoSyncToggle);
        ...
        addButtons: (controls) => {
            controls.appendChild(Utils.createButton(
                `${previewIcon} ${i18n.t('workspaceSwitch')}`,
                async () => {
                    await ChatGPTHandler.switchWorkspace();
                }
            ));
            ...
            if (State.chatgptWorkspaceType === 'user') {
                idLabel.textContent = `${i18n.t('personalWorkspaceHint')}`;
            } else {
